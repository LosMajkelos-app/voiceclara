...
export async function POST(request: NextRequest) {
  try {
    ...
    // Rate limiting - prevent AI credit drain (GPT-4o is expensive!)
    const ip = getClientIp(request)
    const identifier = getRateLimitIdentifier(user.id, ip, 'analyze-results')
    const rateLimitResult = await rateLimit(identifier, RATE_LIMITS.AI_ANALYSIS)
